...
class q_route():
        ...
        for node in self.net.nodes:
            ...
            for neighbour in self.net.nodes[node].neighbour_nodes:
                if neighbour in self.net.gateway_node_ids:
                    self.q_table[node][neighbour] = 100
                else:    
    ...
    def calculate_reward(self,curr_loc,path,msg_len):
        ...
        if next_loc in self.net.gateway_node_ids:
            ...
            reward = -1 + n_e_ratio - n_t_delay - n_d_ratio - n_int_ratio
            reward = (math.sqrt(hop_count))*reward
            if next_loc in path[:-1]:
                reward -= 500
        #print(reward)
        return reward

    def update_q_value(self,node,path,reward):
        max_next_qvalue = max(self.q_table[path[-1]].items(), key=lambda x: x[1])[1]
        q_val = self.q_table[node][path[-1]]
        self.q_table[node][path[-1]] = (1-self.lr)*q_val + self.lr*(reward + self.discount_factor*max_next_qvalue)
